  ...
  return (
    <div className="rounded-xl border border-slate-700 bg-slate-800/40 p-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-sm font-semibold text-slate-200">{t("socket.title")}</h2>
          <p className="text-xs text-slate-500">{t("socket.desc")}</p>
        </div>
        {info?.running ? (
          <button
        ...
        <div className="mt-3 flex items-start gap-4">
          ...
          <div className="space-y-2 text-xs text-slate-400">
            <label className="block">
              <span className="text-slate-300">{t("socket.qrAddress")}</span>
              <input
                type="text"
                value={host}
